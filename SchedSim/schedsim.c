...
//Collaborated with Lauren Guthrie and Ivan Stuart and used ChatGPT as a resource
...
{ 
   ...
   */
    ...
    // Traverse the processes until all are done
    while (1) 
    { 
        int done = 1; 
        ...
        { 
            ...
            { 
                ...
                // If the process's remaining burst time is more than the time quantum
                if (rem_bt[i] > quantum) 
                { 
                    t += quantum; 
                    rem_bt[i] -= quantum; 
...
{ 
    ...
    int total_time = 0, completed = 0, min_index, min_bt;

    // Initialize remaining burst time for each process
    int *bt_rem = (int *)malloc(n * sizeof(int));
    for (int i = 0; i < n; i++)
        bt_rem[i] = plist[i].bt;
    ...
    while (completed < n)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            if (plist[i].art <= total_time && bt_rem[i] < min_bt && bt_rem[i] > 0)
            {
                min_index = i;
                min_bt = bt_rem[i];
            }
        ...
        {
            // Process found, update waiting time, completion time, and remaining burst time
            plist[min_index].wt = total_time - plist[min_index].art;
            total_time += plist[min_index].bt;
            bt_rem[min_index] = 0;
            completed++;
        }
    }
...
{ 
    ...
    ProcessType firstProcess = *(ProcessType *)this;
    ProcessType secondProcess = *(ProcessType *)that;
    if (firstProcess.pri < secondProcess.pri){
      return 1;
    } else {
      return 0;
    }
